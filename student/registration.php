<?php
	...
	if ($currentStuMatNo != "" && $currentStuPas != "") {
		...
		if (isset($_POST['submit']) && !empty($_POST['stateOfOrigin']) && !empty($_POST['localGovt']) && !empty($_FILES['originCertified']['name']) && !empty($_FILES['originCertified']['tmp_name']) && !empty($_FILES['originCertified']['size']) && !empty($_POST['fathersName']) && !empty($_POST['mothersName']) && !empty($_POST['guardian']) && !empty($_POST['guardianMobile']) && !empty($_POST['nextOfKin'])) {
			...
			$guardian = htmlentities($_POST['guardian']);
			$guadianMobile = htmlentities($_POST['guardianMobile']);
			$sponsorMobile = htmlentities($_POST['sponsorMobile']);
			...
			if ($OCError == 0 && $OCExtCon) {
				...
				$insert = mysqli_query($db_conx, "INSERT INTO other_reg(id, matricNo, stateOfOrigin, localGov, placeOfBirth, proveOfOrigin, father, mother, dadMobile, momMobile, sponsor, sponsorMobile, nextOfKin, addressKin, date_made) VALUES(NULL, '$currentStuMatNo', '$stateOfOrigin', '$localGovt', '$placeOfBirth', '$OCName', '$fathersName', '$mothersName', '$fatherMobile', '$motherMobile', '$guardian', '$guadianMobile', '$nextOfKin', '$addressKin', NOW())");
				if ($insert) {
		...
		<form action="<?php echo$_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
			...
			<div id="SSUinput">
				...
				<div>
					...
					<div class="floatL RSAQuery">
						<span class="QHead">Prove of Origin <span class="requiredData">**</span></span>
			...
			<div class="TOApp">Parent & Guardian Information<i class="fas fa-info-circle circleFont"></i></div>
				...
				<div>
					<div class="floatL LSAQuery">
						<span class="QHead">Father's Mobile <span class="requiredData">**</span></span>
						<input type="text" name="fatherMobile" class="NQInput" >
					</div>
					<div class="floatL RSAQuery">
						<span class="QHead">Mother's Mobile <span class="requiredData">**</span></span>
				...
				<div>
					<div class="floatL LSAQuery">
						<span class="QHead">Guardian / Sponsor <span class="requiredData">**</span></span>
						<input type="text" name="guardian" class="TQInput" >
					</div>
					<div class="floatL RSAQuery">
						<span class="QHead">Guardian Mobile <span class="requiredData">**</span></span>
